...
clear_directory() {
    # 指定要清空的文件夹路径
    directory_to_clear="/www/wwwroot/log/git_log/"
...
create_folder_and_file() {
    ...
    # 指定要创建的文件夹路径和文件路径
    folder_to_create="/www/wwwroot/log/git_log/"
...
git_commit() {
    ...
    # 提交到Git仓库
    git add --all
    git commit -m "更新--[log];$commit_message"
